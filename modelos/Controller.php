        ...
        if (isset($_POST['id_unidad_video']) && (isset($_POST['campo_a_editar'])) && (isset($_POST['valor']))) {
            ...
            if (isset($_SESSION['nombre'])) {
                ...
                if ($_POST['campo_a_editar'] == "Estado") {
                    ...
                    $obj_puesto_monitoreo->setCondicion("ID_Unidad_Video=" . $_POST['id_unidad_video']);
                    $obj_puesto_monitoreo->obtiene_puestos_de_monitoreo_presentes_en_unidad_de_video();
                    $params = $obj_puesto_monitoreo->getArreglo();
                    if (count($params) > 0) {
                        for ($i = 0; $i < count($params); $i++) {
                            $obj_puesto_monitoreo->setCondicion("ID_Puesto_Monitoreo=" . $params[$i]['ID_Puesto_Monitoreo'] . " and ID_Unidad_Video=" . $_POST['id_unidad_video']);
                            $obj_puesto_monitoreo->eliminar_registros_puesto_de_monitoreo();
                            $obj_puesto_monitoreo->setCondicion("ID_Puesto_Monitoreo=" . $params[$i]['ID_Puesto_Monitoreo'] . " order by Posicion");
                            $obj_puesto_monitoreo->obtiene_puestos_de_monitoreo_presentes_en_unidad_de_video();
                            $params2 = $obj_puesto_monitoreo->getArreglo();
                            if (count($params2) > 0) {
                                for ($j = 0; $j < count($params2); $j++) {
                                    $obj_puesto_monitoreo->setCondicion("ID_Puesto_Monitoreo_Unidad_Video=" . $params2[$j]['ID_Puesto_Monitoreo_Unidad_Video']);
                                    $nueva_pos = intval($j + 1);
                                    $obj_puesto_monitoreo->setPosicion($nueva_pos);
                                    $obj_puesto_monitoreo->edita_posicion_en_unidades_de_video_de_un_puesto();
                                }
                            }
                        }
                    }
...
//Muestra en pantalla un reportes de eventos cerrados, de acuerdo a parametros de busqueda específicos, como fecha, sitio, etc.
    ...
    public function punto_bcr_guardar() {
        if (isset($_SESSION['nombre'])) {
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                echo ($_GET['id']);
                //Pasa al información del html al dataprovider
                ...
                //Ejecuta la acción de guardar punto BCR
    ...
    /* REVISION DE TABULACIÓN 2017-06-22 1407 */
    ...
    public function persona_guardar_informacion_general() {
        if (isset($_SESSION['nombre'])) {
            $obj_persona = new cls_personal();
            //Almacena la información de la persona en la clase
            ...
            //guarda la condición y actualiza los datos
            $obj_persona->setCondicion("ID_Persona='" . $_POST['id_persona'] . "'");
...
//                                $obj_puesto_monitoreo->obtiene_unidades_no_revisadas();
//                                $unidades_nunca_revisadas=$obj_puesto_monitoreo->getArreglo();
                                ...
                                $puestos_activos = $obj_puesto_monitoreo->getArreglo();
                                $posicion_aleatoria_inicial = $cantidad_puestos_activos;
                                $posicion_aleatoria_final = 50;
                                $pos = 0;
                                ...
                                for ($i = 0; $i < $cantidad_puestos_activos; $i++) {
                                    if ($_SESSION['id'] == $puestos_activos[$i]['ID_Usuario']) {
                                        ...
                                        if (!($obj_puesto_monitoreo->existe_revision_de_video_pendiente_en_bitacora())) {
                                            ...
                                            $pos = rand($posicion_aleatoria_inicial, $posicion_aleatoria_final);
...
//                                $cuenta_sin_revisar=count($unidades_nunca_revisadas);
//                                
//                                echo '<pre>';
//                                echo 'prueba';
//                                print_r($unidades_nunca_revisadas);
//                                echo '</pre>';
//                                        
//                                for ($i = 0; $i < $cuenta_sin_revisar ; $i++) {
//                                    $obj_puesto_monitoreo->setCondicion("Estado=0 and (TIMESTAMPDIFF(MINUTE, concat(Fecha_Inicia_Revision,' ',Hora_Inicia_Revision),NOW())<10) and ID_Unidad_Video=".$unidades_nunca_revisadas[$i]['ID_Unidad_Video']);
//                                    if (!($obj_puesto_monitoreo->existe_revision_de_video_pendiente_en_bitacora())){
//                                            $obj_puesto_monitoreo->setId_unidad_video($unidades_nunca_revisadas[$i]['ID_Unidad_Video']);
//                                            break;
//                                    }
//                                } 
                                //Diez puestos adelante---------------------------------------------------------------------------
                                //Validación que permite enviar un sitio 10 posiciones adelante
//                                $verifica_si_hay_para_revisar=-1;
//                                $verifica_si_hay_para_reacomodar=-1;
//                                if (($_POST['req_mantenimiento']=="1")&&($_POST['res_conexion']=="2")){   
//                                    if (!isset($_SESSION['vector_temp_revision_video_dinamico'])){
//                                        $vector_temp_revision_video_dinamico=array();
//                                        $vector_temp_revision_video_dinamico[]=array("ID_Usuario"=>$_SESSION['id'],"Contador"=>0,"ID_Unidad_Video"=>$_POST['id_u']);
//                                        $_SESSION['vector_temp_revision_video_dinamico']=$vector_temp_revision_video_dinamico;
//                                    }else{
//                                        $tam=count($_SESSION['vector_temp_revision_video_dinamico']);
//                                        $ya_existe_esta_posicion=0;
//                                        for ($i = 0; $i < $tam; $i++) {
//                                            if ($_SESSION['vector_temp_revision_video_dinamico'][$i]['ID_Usuario']==$_SESSION['id']){
//                                                if ($_SESSION['vector_temp_revision_video_dinamico'][$i]['ID_Unidad_Video']==$_POST['id_u']){
//                                                    $ya_existe_esta_posicion=1; 
//                                                }
//                                                $temp=intval($_SESSION['vector_temp_revision_video_dinamico'][$i]['Contador']);
//                                                $temp=$temp+1;
//                                                $_SESSION['vector_temp_revision_video_dinamico'][$i]['Contador']=$temp;
//
//                                                //if ($_SESSION['vector_temp_revision_video_dinamico'][$i]['Contador']<11){
//                                                //    $_SESSION['vector_temp_revision_video_dinamico'][$i]['ID_Unidad_Video']=$_POST['id_u'];
//                                                //}
//                                                if ($_SESSION['vector_temp_revision_video_dinamico'][$i]['Contador']==10){
//                                                    $verifica_si_hay_para_revisar=$i;
//                                                }
//                                                if ($_SESSION['vector_temp_revision_video_dinamico'][$i]['Contador']==11){
//                                                    $verifica_si_hay_para_reacomodar=$i;
...
//                                        if ($ya_existe_esta_posicion==0){
//                                            $vector_temp_revision_video_dinamico[]=array("ID_Usuario"=>$_SESSION['id'],"Contador"=>0,"ID_Unidad_Video"=>$_POST['id_u']);
//                                            $_SESSION['vector_temp_revision_video_dinamico']=  array_merge($_SESSION['vector_temp_revision_video_dinamico'],$vector_temp_revision_video_dinamico);
//                                        }
//                                    }    
//                                }else{
//                                    if (isset($_SESSION['vector_temp_revision_video_dinamico'])){
...
//                                }
//                                if ($verifica_si_hay_para_revisar>-1){
//                                    $obj_unidades_de_video->setCondicion("ID_Unidad_Video=".$_SESSION['vector_temp_revision_video_dinamico'][$verifica_si_hay_para_revisar]['ID_Unidad_Video']. " AND Estado=0");
//                                    if ($obj_unidades_de_video->existe_este_dato_en_la_tabla_unidades_video()){
//                                        $obj_puesto_monitoreo->setId_unidad_video($_SESSION['vector_temp_revision_video_dinamico'][$verifica_si_hay_para_revisar]['ID_Unidad_Video']);
//                                    }
//                                    $verifica_si_hay_para_revisar=-1;
//                                }  
//                                if ($verifica_si_hay_para_reacomodar>-1){  
//                                   
//                                    unset($_SESSION['vector_temp_revision_video_dinamico'][$verifica_si_hay_para_reacomodar]);
//                                    if(count($_SESSION['vector_temp_revision_video_dinamico'])==0){
//                                        unset($_SESSION['vector_temp_revision_video_dinamico']);
//                                    }else{
//                                        $_SESSION['vector_temp_revision_video_dinamico'] = array_values($_SESSION['vector_temp_revision_video_dinamico']);
//
//                                    }
//                                    $verifica_si_hay_para_reacomodar=-1;
//                                }
                                //Termina diez puestos adelante------------------------------------------------------------------
...
//                                         } 
    ...
    public function controles_de_video_listar() {
        if (isset($_SESSION['nombre'])) {
            ...
            //echo '<pre>';
            //print_r ($_SESSION['vector_temp_revision_video_dinamico']);
            //echo '</pre>';
                ...
                if (!(count($vector_revision_de_video_actual) > 0)) {
                    ...
                    if (!(count($obj_puesto_monitoreo->getArreglo()) > 0)) {
                        ...
                        if (count($obj_puesto_monitoreo->getArreglo()) > 0) {
                            ...
                            $obj_puesto_monitoreo->setCondicion("ID_Puesto_Monitoreo=" . $vector_puesto_de_monitoreo_actual[0]['ID_Puesto_Monitoreo'] . " AND Posicion=" . $temp_posicion);

                            if ($obj_puesto_monitoreo->existe_esta_posicion_en_este_puesto_de_monitoreo()) {
                                //$obj_puesto_monitoreo->setPosicion($obj_puesto_monitoreo->getUltima_posicion_concluida()+1);
                                $obj_puesto_monitoreo->setPosicion($temp_posicion);
                            } else {
                                $obj_puesto_monitoreo->setCondicion("ID_Puesto_Monitoreo=" . $vector_puesto_de_monitoreo_actual[0]['ID_Puesto_Monitoreo'] . " AND Posicion=1");
                                if ($obj_puesto_monitoreo->existe_esta_posicion_en_este_puesto_de_monitoreo()) {
                                    $obj_puesto_monitoreo->setPosicion(1);
                                }
                            }
                            ...
                            //Ingresa nuevo registro de bitacora en la tabla de revisiones de video.
                            $obj_puesto_monitoreo->setId_ultimo_toma_puesto_ingresada($vector_bitacora_control_de_puesto[0]['ID_Bitacora_Control_Puesto_Monitoreo']);
                            $obj_puesto_monitoreo->setFecha_inicia_revision(date("Y-m-d"));
                            $obj_puesto_monitoreo->setHora_inicia_revision(date("H:i:s", time()));
                            $obj_puesto_monitoreo->setId_usuario($_SESSION['id']);
...
//                                    }
    ...
    public function alertas_generales() {
        if (isset($_SESSION['nombre'])) {
            ...
            //Obtiene la información de la ultima revisión de cada unidad de video
            $obj_reporteria->setCondicion("");
            $obj_reporteria->ultima_revision_por_unidad_video_simple();
            $ultima_revision = $obj_reporteria->getArreglo();
...
//            echo "</pre>";
    ...
    public function reporte_ultimas_revisiones_video_completo() {
        if (isset($_SESSION['nombre'])) {
            $obj_reporteria = new cls_reporteria();
            $obj_usuario = new cls_usuarios();
            ...
            //Ciclo para obtener el promedio de revisiones general
            $promedio_general = 0;
            for ($i = 0; $i < $tam; $i++) {
            ...
            $minutos = floor(($promedio_general - ($horas * 3600)) / 60);
            $segundos = $promedio_general - ($horas * 3600) - ($minutos * 60);
            if ($horas > 0) {
                $hora_texto .= $horas . "h ";
            }
            if ($minutos > 0) {
    ...
    public function reporte_tiempo_revision_actual() {
        ...
        for ($i = 0; $i < count($bitacora_revision_video); $i++) {
            ...
            $diff = $fecha1->diff($fecha2);
            if (($diff->h) > 0) {
                $tiempo_transcurrido = ($diff->h . "h: " . $diff->i . "m." . $diff->s . "s.");
            } else {
                $tiempo_transcurrido = ($diff->i . "m." . $diff->s . "s.");
            }
    ...
    public function revision_controles_desatendidos() {
        $obj_reporteria = new cls_reporteria();
        $obj_traza = new cls_trazabilidad();
        ...
        if ($_SESSION['rol'] == 1 || $_SESSION['rol'] == 11) {
            for ($i = 0; $i < count($bitacora_revision_video); $i++) {
                ...
                if ($bitacora_revision_video[$i]['ID_Puesto_Monitoreo'] >= 1 && $bitacora_revision_video[$i]['ID_Puesto_Monitoreo'] <= 4 || $bitacora_revision_video[$i]['ID_Puesto_Monitoreo'] == 15) {
                    if ($bitacora_revision_video[$i]['Control'] == 0 && $suma_tiempos >= 10) {
                        $data .= "-" . $bitacora_revision_video[$i]['Nombre'] . " Libre por más de 10 min <br>";
                    }
                }
                if ($bitacora_revision_video[$i]['ID_Puesto_Monitoreo'] == 6) {
                    if (date("H:i:s", $time) >= '06:30:00' && date("H:i:s", $time) <= '19:30:00' && $bitacora_revision_video[$i]['Control'] == 0 && $suma_tiempos >= 10) {
                        $data .= "-" . $bitacora_revision_video[$i]['Nombre'] . " Libre por más de 10 min <br>";
                    }
                }
                if ($bitacora_revision_video[$i]['ID_Puesto_Monitoreo'] == 7) {
                    if (date("H:i:s", $time) >= '08:00:00' && date("H:i:s", $time) <= '16:00:00' && $bitacora_revision_video[$i]['Control'] == 0 && $suma_tiempos >= 10) {
                        $data .= "-" . $bitacora_revision_video[$i]['Nombre'] . " Libre por más de 10 min <br>";
                    }
                }
                if ($bitacora_revision_video[$i]['ID_Puesto_Monitoreo'] == 8) {
                    if (date("H:i:s", $time) >= '10:00:00' && date("H:i:s", $time) <= '18:00:00' && $bitacora_revision_video[$i]['Control'] == 0 && $suma_tiempos >= 10) {
                        $data .= "-" . $bitacora_revision_video[$i]['Nombre'] . " Libre por más de 10 min <br>";
                    }
                }
                if ($bitacora_revision_video[$i]['ID_Puesto_Monitoreo'] == 9) {
                    if ((date("H:i:s", $time) <= '07:00:00' || date("H:i:s", $time) >= '12:00:00') && $bitacora_revision_video[$i]['Control'] == 0 && $suma_tiempos >= 10) {
                        $data .= "-" . $bitacora_revision_video[$i]['Nombre'] . " Libre por más de 10 min <br>";
                    }
                }
                if ($bitacora_revision_video[$i]['ID_Puesto_Monitoreo'] == 10) {
                    if (date("H:i:s", $time) >= '12:00:00' && date("H:i:s", $time) <= '22:00:00' && $bitacora_revision_video[$i]['Control'] == 0 && $suma_tiempos >= 10) {
                        $data .= "-" . $bitacora_revision_video[$i]['Nombre'] . " Libre por más de 10 min <br>";
                    }
                }
                if ($bitacora_revision_video[$i]['Control'] > 0 && $suma_tiempos > 10) {
                    if ($suma_tiempos > 30) {
                        $data .= "-" . $bitacora_revision_video[$i]['Nombre'] . " desatendido por más de 30 min<br>";
                    } else {
                        $obj_traza->setCondicion("(t_traza.ID_Usuario=" . $bitacora_revision_video[$i]['ID_Usuario'] . ") AND (t_traza.Tabla_Afectada='T_Evento' OR t_traza.Tabla_Afectada='T_detalleEvento' OR t_traza.Tabla_Afectada='T_EventoCencon' OR t_traza.Tabla_Afectada='T_PruebaAlarma') AND (t_traza.Fecha between '" . $bitacora_revision_video[$i]['Fecha_Inicia_Revision'] . "' AND '" . date("Y-m-d") . "') AND (t_traza.Hora between '" . $bitacora_revision_video[$i]['Hora_Inicia_Revision'] . "' AND '" . date("H:i:s", time()) . "')");
                        $obj_traza->obtiene_trazabilidad();
                        if (count($obj_traza->getArreglo()) == 0) {
                            $data .= "-" . $bitacora_revision_video[$i]['Nombre'] . " desatendido por más de 10 min<br>";
                        }
                    }
                }
            }
    ...
    public function buscar_punto_prueba_alarma() {
        if (isset($_SESSION['nombre'])) {
            ...
            $obj_eventos->setCondicion("T_Evento.ID_PuntoBCR=" . $puntobcr[0]['ID_PuntoBCR'] . " AND T_Evento.ID_Tipo_Evento IN (22,40) AND T_Evento.ID_EstadoEvento='1' AND T_Evento.Fecha='" . date('Y-m-j') . "'");
            $obj_eventos->obtiene_todos_los_eventos();
...
//                    }
    ...
    public function actualizar_controladores_paso_1() {
        if (isset($_SESSION['nombre'])) {
            ...
            for ($i = 0; $i < count($controladores); $i++) {
                $controladores[$i][0] = str_replace("\\", "/", $controladores[$i][0]);
                $controladores[$i][0] = str_replace(" ", "", $controladores[$i][0]);
            }
        ...
        if (isset($_SESSION['nombre'])) {
            ...
            for ($i = 0; $i < count($controladores_subidos); $i++) {
                if ($controladores_subidos[$i][1] <> "0") {
                    $correo .= "El siguiente controlador es nuevo en la base de datos " . $controladores_subidos[$i]['1'] . ".\r\n";
                    $obj_controlador->setOwner($controladores_subidos[$i][0]);
                    $obj_controlador->setName($controladores_subidos[$i][1]);
                    $obj_controlador->setNetworkid($controladores_subidos[$i][2]);
                    $obj_controlador->setIpaddress($controladores_subidos[$i][3]);
    ...
    public function correo_actualizacion_controlador($detalle, $tipo) {
        if (isset($_SESSION['nombre'])) {
            $obj_usuario = new cls_usuarios();
            $obj_correo = new Mail_Provider();
            ...
            $ruta = $raiz . "Cuenta_Visitas_Oriel/Control_Acceso/" . date("Ymd_his", $time) . "_" . $tipo . ".txt";
            if (!file_exists($ruta)) {
                $fp = fopen($ruta, "a+");
                //Escribe en el archivo
                fwrite($fp, $detalle);
     ...
     */
    public function andru_preguntas_listar() {
        if (isset($_SESSION['nombre'])) {
            $obj_andru_preguntas = new cls_andru_preguntas();
            $obj_andru_fases = new cls_andru_fases();
            $obj_andru_catagoria = new cls_andru_categoria();
            $obj_tipos_porcentaje = new cls_andru_tipos_porcentajes();

            //Procede a ejecutar la consulta SQL para traer todo de t_andru_preguntas en la bd.
            $obj_andru_preguntas->setCondicion("");
            //Obtener el vector de la consulta
            $obj_andru_preguntas->obtener_andru_preguntas();
            $andru_preguntas = $obj_andru_preguntas->getArreglo();

            //Fases
            $obj_andru_fases->setCondicion("Estado = 1");
            $obj_andru_fases->obtener_andru_fases();
            $andru_fases = $obj_andru_fases->getArreglo();

            //Categorias
            $obj_andru_catagoria->setCondicion("Estado =1");
            $obj_andru_catagoria->obtener_andru_categoria();
            $andru_catagorias = $obj_andru_catagoria->getArreglo();

            //Tipos Porcentaje
            $obj_tipos_porcentaje->setCondicion("Estado =1");
            $obj_tipos_porcentaje->obtener_andru_tipos_porcentajes();
            $andru_tiposporcentaje = $obj_tipos_porcentaje->getArreglo();

            unset($obj_andru_preguntas);
            unset($obj_andru_fases);
            unset($obj_andru_catagoria);
            unset($obj_tipos_porcentaje);
            require __DIR__ . '/../vistas/plantillas/frm_andru_preguntas.php';
        } else {
            $tipo_de_alerta = "alert alert-warning";
            $validacion = "Es necesario volver a iniciar sesión para consultar el sistema";
            //Llamada al formulario correspondiente de la vista
    ...
    /**
     * Método que guarda en base datos cuando la propiedad ID_Pregunta esta en cero 
     * Caso contrario actualiza en base datos el registros según el valor de ID_Pregunta 
     */
    public function andru_preguntas_guardar() {
        if (isset($_SESSION['nombre'])) {
            $obj_andru_preguntas = new cls_andru_preguntas();
            $obj_andru_preguntas_Porcentajes = new cls_andru_preguntas_porcentajes();
            $obj_andru_preguntas_Respuestas = new cls_andru_preguntas_respuestas();

            $rowID = null;
            $condicion = "";
            //Almaceno los valores
            $obj_andru_preguntas->setID_Fase($_POST['ID_Fase']);
            $obj_andru_preguntas->setID_Categoria($_POST['ID_Categoria']);
            $obj_andru_preguntas->setDescripcion($_POST['Descripcion']);
            $obj_andru_preguntas->setEstado($_POST['Estado']);

            //Guardo la pregunta
            if ($_POST['ID_Pregunta'] == 0) {
                $obj_andru_preguntas->guardar_andru_preguntas();
                $condicion .= "ID_Fase ='" . $obj_andru_preguntas->getID_Fase() . "' AND ";
                $condicion .= "ID_Categoria ='" . $obj_andru_preguntas->getID_Categoria() . "'AND ";
                $condicion .= "Descripcion ='" . $obj_andru_preguntas->getDescripcion() . "'AND ";
                $condicion .= "Estado ='" . $obj_andru_preguntas->getEstado() . "' LIMIT 1 ";

                //Busco el ID de la pregunta insertada
                $obj_andru_preguntas->setCondicion($condicion);
                $obj_andru_preguntas->obtener_andru_preguntas();
                $rowID = $obj_andru_preguntas->getArreglo();
                $obj_andru_preguntas->setID_Pregunta(0);

                $tam = count($rowID);
                if ($tam > 0) {
                    $obj_andru_preguntas->setID_Pregunta($rowID[0]["ID_Pregunta"]);
                }
                $rowID = null;
            } else {
                $obj_andru_preguntas->setID_Pregunta($_POST['ID_Pregunta']);
                $obj_andru_preguntas->setCondicion("ID_Pregunta='" . $_POST['ID_Pregunta'] . "'");
                $obj_andru_preguntas->guardar_andru_preguntas();
            }

            if ($obj_andru_preguntas->getID_Pregunta() > 0) {
                //Guardo el porcentaje para la pregunta
                $tblsID = $_POST['tblId'];
                $tblsDesc = $_POST['tbldesc'];
                $tam = count($tblsID);

                for ($i = 0; $i < $tam; $i++) {
                    $condicion = "";
                    $rowID = null;
                    $obj_andru_preguntas_Porcentajes->setID_Pregunta($obj_andru_preguntas->getID_Pregunta());
                    $obj_andru_preguntas_Porcentajes->setID_Tipo_Porcentaje($tblsID[$i]);
                    $obj_andru_preguntas_Porcentajes->setValor($tblsDesc[$i]);

                    $condicion .= "ID_Pregunta ='" . $obj_andru_preguntas_Porcentajes->getID_Pregunta() . "' AND ";
                    $condicion .= "ID_Tipo_Porcentaje ='" . $obj_andru_preguntas_Porcentajes->getID_Tipo_Porcentaje() . "' ";

                    //Valido que no se ingresen repetidos
                    $obj_andru_preguntas_Porcentajes->setCondicion($condicion);
                    $obj_andru_preguntas_Porcentajes->obtener_andru_preguntas_porcentajes();
                    $rowID = $obj_andru_preguntas_Porcentajes->getArreglo();
                    $tam2 = count($rowID);

                    if ($tam2 == 0) {
                        $obj_andru_preguntas_Porcentajes->setCondicion("");
                    }
                    $obj_andru_preguntas_Porcentajes->guardar_andru_preguntas_porcentajes();
                }

                //Guardo las posibles respuestas
                $tblsIDRes = $_POST['tblIdRes'];
                $tblsDescRes = $_POST['tbldescRes'];
                $tblsEstadoRes = $_POST['tblestadoRes'];
                $tblsNivelRes = $_POST['tblnivelRes'];
                $tam = count($tblsIDRes);

                for ($i = 0; $i < $tam; $i++) {
                    $condicion = "";
                    $rowID = null;
                    $obj_andru_preguntas_Respuestas->setID_Pregunta($obj_andru_preguntas->getID_Pregunta());
                    $obj_andru_preguntas_Respuestas->setDescripcion($tblsDescRes[$i]);
                    $obj_andru_preguntas_Respuestas->setNivel($tblsNivelRes[$i]);
                    $obj_andru_preguntas_Respuestas->setEstado($tblsEstadoRes[$i]);

                    $condicion .= "ID_Pregunta ='" . $obj_andru_preguntas_Respuestas->getID_Pregunta() . "' AND ";
                    $condicion .= "ID_Respuesta ='" . $tblsIDRes[$i] . "' ";

                    if ($tblsIDRes[$i] != "-1") {
                        $obj_andru_preguntas_Respuestas->setCondicion($condicion);
                        $obj_andru_preguntas_Respuestas->setID_Respuesta($tblsIDRes[$i]);
                    } else {
                        $obj_andru_preguntas_Respuestas->setCondicion("");
                        $obj_andru_preguntas_Respuestas->setID_Respuesta(0);
                    }

                    if ($obj_andru_preguntas_Respuestas->getDescripcion() != "") {
                        $obj_andru_preguntas_Respuestas->guardar_andru_preguntas_respuestas();
                    }
                }
            }

            unset($rowID);
            unset($obj_andru_preguntas_Respuestas);
            unset($obj_andru_preguntas_Porcentajes);
            unset($obj_andru_preguntas);
            header("location:/ORIEL/index.php?ctl=andru_preguntas_listar");
        } else {
            $tipo_de_alerta = "alert alert-warning";
            $validacion = "Es necesario volver a iniciar sesión para consultar el sistema";
            require __DIR__ . '/../vistas/plantillas/inicio_sesion.php';
    ...
    public function andru_preguntas_porcentajes_trae() {
        if (isset($_SESSION['nombre'])) {
            $obj_preguntas_porcentajes = new cls_andru_preguntas_porcentajes();
            $obj_preguntas_porcentajes->setCondicion("ID_Pregunta='" . $_POST['id'] . "' ");
            $obj_preguntas_porcentajes->obtener_andru_preguntas_porcentajes();
            $DatosProcentajes = $obj_preguntas_porcentajes->getArreglo();

            //Convierte la información en un json para enviarlo a JavaScript
            unset($obj_preguntas_porcentajes);

            if ($DatosProcentajes[0] != null) {
                echo json_encode($DatosProcentajes, JSON_FORCE_OBJECT);
            } else {
                echo "No se encontró";
            }
        } else {
    ...
    public function andru_preguntas_respuestas_trae() {
        if (isset($_SESSION['nombre'])) {
            $obj_preguntas_respuestas = new cls_andru_preguntas_respuestas();
            $obj_preguntas_respuestas->setCondicion("ID_Pregunta='" . $_POST['id'] . "' ");
            $obj_preguntas_respuestas->obtener_andru_preguntas_respuestas();
            $DatosRespuestas = $obj_preguntas_respuestas->getArreglo();

            //Convierte la información en un json para enviarlo a JavaScript
            unset($obj_preguntas_respuestas);

            if ($DatosRespuestas[0] != null) {
                echo json_encode($DatosRespuestas, JSON_FORCE_OBJECT);
            } else {
                echo "No se encontró";
            }
        } else {
     ...
     */
    public function andru_preguntas_cambiar_estado() {
        if (isset($_SESSION['nombre'])) {
            $obj_andru_preguntas = new cls_andru_preguntas();
            if ($_GET['Estado'] == 1) {
                $obj_andru_preguntas->setEstado("0");
            } else {
                $obj_andru_preguntas->setEstado("1");
            }
            $obj_andru_preguntas->setCondicion("ID_Pregunta='" . $_GET['ID_Pregunta'] . "'");
            $obj_andru_preguntas->cambiar_estado_andru_preguntas();
     ...
     * Método que retorna un arreglo de los registros que existen en base de datos
     */
    public function andru_fases_listar() {
        if (isset($_SESSION['nombre'])) {
            $obj_andru_fases = new cls_andru_fases();
            ...
            require __DIR__ . '/../vistas/plantillas/frm_andru_fases.php';
        } else {
            $tipo_de_alerta = "alert alert-warning";
            $validacion = "Es necesario volver a iniciar sesión para consultar el sistema";
            //Llamada al formulario correspondiente de la vista
    ...
    /**
     * Método que guarda en base datos cuando la propiedad ID_Fase esta en cero 
     * Caso contrario actualiza en base datos el registros según el valor de ID_Fase 
     */
    public function andru_fases_guardar() {
        if (isset($_SESSION['nombre'])) {
            $obj_andru_fases = new cls_andru_fases();
            $obj_andru_fases->setDescripcion($_POST['Descripcion']);

            $obj_andru_fases->setEstado($_POST['Estado']);
            if ($_POST['ID_Fase'] != 0) {
                $obj_andru_fases->setID_Fase($_POST['ID_Fase']);
                $obj_andru_fases->setCondicion("ID_Fase='" . $_POST['ID_Fase'] . "'");
            }
            $obj_andru_fases->guardar_andru_fases();
            unset($obj_andru_fases);
            header("location:/ORIEL/index.php?ctl=andru_fases_listar");
        } else {
            $tipo_de_alerta = "alert alert-warning";
            $validacion = "Es necesario volver a iniciar sesión para consultar el sistema";
            require __DIR__ . '/../vistas/plantillas/inicio_sesion.php';
     ...
     * Método que se utiliza para cambiar el estado del registro según el campo llave 
     */
    public function andru_fases_cambiar_estado() {
        if (isset($_SESSION['nombre'])) {
            $obj_andru_fases = new cls_andru_fases();
            if ($_GET['Estado'] == 1) {
                $obj_andru_fases->setEstado("0");
            } else {
                $obj_andru_fases->setEstado("1");
            }
            $obj_andru_fases->setCondicion("ID_Fase='" . $_GET['ID_Fase'] . "'");
            $obj_andru_fases->cambiar_estado_andru_fases();
            unset($obj_andru_fases);
            header("location:/ORIEL/index.php?ctl=andru_fases_listar");
        } else {
            $tipo_de_alerta = "alert alert-warning";
     ...
     * Método que retorna un arreglo de los registros que existen en base de datos
     */
    public function andru_categoria_listar() {
        if (isset($_SESSION['nombre'])) {
            $obj_andru_categoria = new cls_andru_categoria();
            //Procede a ejecutar la consulta SQL para traer todo de t_andru_categoria en la bd.
            $obj_andru_categoria->setCondicion("");
            //Obtener el vector de la consulta
            $obj_andru_categoria->obtener_andru_categoria();
            $andru_categoria = $obj_andru_categoria->getArreglo();
            unset($obj_andru_categoria);

            require __DIR__ . '/../vistas/plantillas/frm_andru_categoria.php';
        } else {
            $tipo_de_alerta = "alert alert-warning";
            $validacion = "Es necesario volver a iniciar sesión para consultar el sistema";
            //Llamada al formulario correspondiente de la vista
    ...
    /**
     * Método que guarda en base datos cuando la propiedad ID_Categoria esta en cero 
     * Caso contrario actualiza en base datos el registros según el valor de ID_Categoria 
     */
    public function andru_categoria_guardar() {
        if (isset($_SESSION['nombre'])) {
            $obj_andru_categoria = new cls_andru_categoria();
            $obj_andru_categoria->setDescripcion($_POST['Descripcion']);

            $obj_andru_categoria->setEstado($_POST['Estado']);
            if ($_POST['ID_Categoria'] != 0) {
                $obj_andru_categoria->setID_Categoria($_POST['ID_Categoria']);
                $obj_andru_categoria->setCondicion("ID_Categoria='" . $_POST['ID_Categoria'] . "'");
            }
            $obj_andru_categoria->guardar_andru_categoria();
            unset($obj_andru_categoria);
            header("location:/ORIEL/index.php?ctl=andru_categoria_listar");
        } else {
            $tipo_de_alerta = "alert alert-warning";
            $validacion = "Es necesario volver a iniciar sesión para consultar el sistema";
            require __DIR__ . '/../vistas/plantillas/inicio_sesion.php';
     ...
     * Método que se utiliza para cambiar el estado del registro según el campo llave 
     */
    public function andru_categoria_cambiar_estado() {
        if (isset($_SESSION['nombre'])) {
            $obj_andru_categoria = new cls_andru_categoria();
            if ($_GET['Estado'] == 1) {
                $obj_andru_categoria->setEstado("0");
            } else {
                $obj_andru_categoria->setEstado("1");
            }
            $obj_andru_categoria->setCondicion("ID_Categoria='" . $_GET['ID_Categoria'] . "'");
            $obj_andru_categoria->cambiar_estado_andru_categoria();
            unset($obj_andru_categoria);
            header("location:/ORIEL/index.php?ctl=andru_categoria_listar");
        } else {
            $tipo_de_alerta = "alert alert-warning";
    ...
    ////////////////////////////////////////////////////////////////////////////
    //////////////////////////////andru_tipos_porcentajes
    ////////////////////////////////////////////////////////////////////////////
    /**
     * Método que retorna un arreglo de los registros que existen en base de datos
     */
    public function andru_tipos_porcentajes_listar() {
        if (isset($_SESSION['nombre'])) {
            $obj_andru_tipos_porcentajes = new cls_andru_tipos_porcentajes();
            //Procede a ejecutar la consulta SQL para traer todo de t_andru_tipos_porcentajes en la bd.
            $obj_andru_tipos_porcentajes->setCondicion("");
            //Obtener el vector de la consulta
            $obj_andru_tipos_porcentajes->obtener_andru_tipos_porcentajes();
            $andru_tipos_porcentajes = $obj_andru_tipos_porcentajes->getArreglo();
            unset($obj_andru_tipos_porcentajes);

            require __DIR__ . '/../vistas/plantillas/frm_andru_tipos_porcentajes.php';
        } else {
            $tipo_de_alerta = "alert alert-warning";
            $validacion = "Es necesario volver a iniciar sesión para consultar el sistema";
            //Llamada al formulario correspondiente de la vista
    ...
    /**
     * Método que guarda en base datos cuando la propiedad ID_Tipo_Porcentaje esta en cero 
     * Caso contrario actualiza en base datos el registros según el valor de ID_Tipo_Porcentaje 
     */
    public function andru_tipos_porcentajes_guardar() {
        if (isset($_SESSION['nombre'])) {
            $obj_andru_tipos_porcentajes = new cls_andru_tipos_porcentajes();
            $obj_andru_tipos_porcentajes->setDescripcion($_POST['Descripcion']);

            $obj_andru_tipos_porcentajes->setEstado($_POST['Estado']);
            if ($_POST['ID_Tipo_Porcentaje'] != 0) {
                $obj_andru_tipos_porcentajes->setID_Tipo_Porcentaje($_POST['ID_Tipo_Porcentaje']);
                $obj_andru_tipos_porcentajes->setCondicion("ID_Tipo_Porcentaje='" . $_POST['ID_Tipo_Porcentaje'] . "'");
            }
            $obj_andru_tipos_porcentajes->guardar_andru_tipos_porcentajes();
            unset($obj_andru_tipos_porcentajes);
            header("location:/ORIEL/index.php?ctl=andru_tipos_porcentajes_listar");
        } else {
            $tipo_de_alerta = "alert alert-warning";
            $validacion = "Es necesario volver a iniciar sesión para consultar el sistema";
            require __DIR__ . '/../vistas/plantillas/inicio_sesion.php';
     ...
     * Método que se utiliza para cambiar el estado del registro según el campo llave 
     */
    public function andru_tipos_porcentajes_cambiar_estado() {
        if (isset($_SESSION['nombre'])) {
            $obj_andru_tipos_porcentajes = new cls_andru_tipos_porcentajes();
            if ($_GET['Estado'] == 1) {
                $obj_andru_tipos_porcentajes->setEstado("0");
            } else {
                $obj_andru_tipos_porcentajes->setEstado("1");
            }
            $obj_andru_tipos_porcentajes->setCondicion("ID_Tipo_Porcentaje='" . $_GET['ID_Tipo_Porcentaje'] . "'");
            $obj_andru_tipos_porcentajes->cambiar_estado_andru_tipos_porcentajes();
            unset($obj_andru_tipos_porcentajes);
            header("location:/ORIEL/index.php?ctl=andru_tipos_porcentajes_listar");
        } else {
            $tipo_de_alerta = "alert alert-warning";
     ...
     */
    public function andru_cuestionario() {
        if (isset($_SESSION['nombre'])) {
            $obj_andru_cuestionario = new cls_andru_cuestionario();
            $obj_andru_fase = new cls_andru_fases();
            $obj_andru_puntobcr = new cls_puntosBCR();
            $obj_andru_preguntas = new cls_andru_cuestionario();
            $obj_tipos_porcentajes = new cls_andru_tipos_porcentajes();

            $preguntas = null;
            $idcuestionario = 0;
            $idpuntobcr = 0;
            $idfase = 0;
            $id_preguntasvec = array();
            $id_respuestasvec = array();

            //Tipos de porcentajes
            $condicion = " Estado = 1 ORDER BY ID_Tipo_Porcentaje ASC";
            $obj_tipos_porcentajes->setCondicion($condicion);
            $obj_tipos_porcentajes->obtener_andru_tipos_porcentajes();
            $tipos_porcentajes = $obj_tipos_porcentajes->getArreglo();

            //Oficinas BCR Activos
            $condicion = "ID_Tipo_Punto = 1 AND Estado = 1 ";
            $obj_andru_puntobcr->setCondicion($condicion);
            $obj_andru_puntobcr->obtiene_solo_los_puntos_bcr();
            $puntosbcr = $obj_andru_puntobcr->getArreglo();

            //Fases BCR Activas
            $condicion = "Estado = 1 ";
            $obj_andru_fase->setCondicion($condicion);
            $obj_andru_fase->obtener_andru_fases();
            $fasesbcr = $obj_andru_fase->getArreglo();

            //obtiene el tipo ID_Cuestionario
            if (isset($_GET['idcues'])) {
                $idcuestionario = $_GET['idcues'];
            }

            if ($idcuestionario == 0) {
                $nom_puntobcr = "Ninguno";
                $nom_fase = "Ninguno";
            }

            //obtiene el tipo ID_PuntoBCR
            if (isset($_GET['idpunto'])) {
                $idpuntobcr = $_GET['idpunto'];
                if ($idpuntobcr > 0) {
            ...
            //obtiene las respuestas y porcentajes por pregunta para mostar en el html
            if (($idfase != 0) && ($idpuntobcr != 0)) {

                $idcuestionariovec = $this->BuscarIdCuestionario($idpuntobcr, $idfase);
                $idcuestionario = $idcuestionariovec[0];
                ...
                $obj_andru_preguntas->setCondicion($condicion);
                $obj_andru_preguntas->obtener_andru_cuestionario_respuesta();
                $respuestas = $obj_andru_preguntas->getArreglo();
                //Obtengo las porcentajes por pregunta
                $condicion = " p.ID_Fase = " . $idfase . " AND p.Estado = 1 ORDER BY por.ID_Pregunta, por.ID_Tipo_Porcentaje ASC;";
                $obj_andru_preguntas->setCondicion($condicion);
                $obj_andru_preguntas->obtener_andru_cuestionario_porcentaje();
                $preguntasporcentaje = $obj_andru_preguntas->getArreglo();
                //Obtengo las porcentajes por pregunta según respuesta
                $condicion = " tp.Estado = 1 ORDER BY cr.ID_Pregunta,tp.ID_Tipo_Porcentaje ASC;";
                $obj_andru_preguntas->setCondicion($condicion);
            ...
            unset($obj_tipos_porcentajes);
            unset($obj_andru_preguntas);
            unset($obj_andru_fase);
            unset($obj_andru_cuestionario);
            unset($obj_andru_puntobcr);
            require __DIR__ . '/../vistas/plantillas/frm_andru_cuestionario.php';
        } else {
            $tipo_de_alerta = "alert alert-warning";
            $validacion = "Es necesario volver a iniciar sesión para consultar el sistema";
            //Llamada al formulario correspondiente de la vista
    ...
    /**
     * Método que guarda en base datos cuando la propiedad ID_Cuestionario esta en cero 
     * Caso contrario actualiza en base datos el registros según el valor de ID_Cuestionario 
     */
    public function andru_cuestionario_guardar() {
        if (isset($_SESSION['nombre'])) {

            //Asigno las variables a la clase
            $obj_andru_cuestionario = new cls_andru_cuestionario();
            $obj_andru_cuestionario_respts = new cls_andru_cuestionario_respuestas();
            $condicion = "";

            $obj_andru_cuestionario->setID_PuntoBCR($_POST['ID_PuntoBCR']);
            $obj_andru_cuestionario->setID_Fase($_POST['ID_Fase']);
            $obj_andru_cuestionario->setUsuario_Crea($_SESSION['id']);
            $obj_andru_cuestionario->setFecha_Crea(date('Y/m/d H:i'));
            $obj_andru_cuestionario->setUsuario_Modifica($_SESSION['id']);
            $obj_andru_cuestionario->setFecha_Modifica(date('Y/m/d H:i'));
            $obj_andru_cuestionario->setEstado("1");

            $obj_andru_cuestionario->setID_Cuestionario(0);

            //revisio si existe un registro guardado            
            $v_resultado = $this->BuscarIdCuestionario($_POST['ID_PuntoBCR'], $_POST['ID_Fase']);
            if (count($v_resultado) > 0) {
                $obj_andru_cuestionario->setID_Cuestionario($v_resultado[0]);
                $obj_andru_cuestionario->setFecha_Crea($v_resultado[1]);
                $obj_andru_cuestionario->setUsuario_Crea($v_resultado[2]);
                $condicion = "ID_Cuestionario = " . $obj_andru_cuestionario->getID_Cuestionario();
                $obj_andru_cuestionario->setCondicion($condicion);
            }

            //guarda la información del encabezado del formulario
            $obj_andru_cuestionario->guardar_andru_cuestionario();

            //busco el ID_Cuestionario insertado según el punto y la fase
            if ($obj_andru_cuestionario->getID_Cuestionario() == 0) {
                $v_resultado = $this->BuscarIdCuestionario($_POST['ID_PuntoBCR'], $_POST['ID_Fase']);
                if (count($v_resultado) > 0) {
                    $obj_andru_cuestionario->setID_Cuestionario($v_resultado[0]);
                    $obj_andru_cuestionario->setFecha_Crea($v_resultado[1]);
            ...
            if ($obj_andru_cuestionario->getID_Cuestionario() != 0) {
                ...
                $preguntas = $_POST['id_preguntas'];
                $respuestas = $_POST['id_respuestas'];

                $preguntasvec = explode(',', $preguntas);
            ...
            unset($obj_andru_cuestionario_respts);
            unset($obj_andru_cuestionario);
    ...
    /*
     * Método que busca el ID_Cuestionario según la fase y el punto BCR
     */

    public function BuscarIdCuestionario($pIdPuntoBCR, $pIdFase) {
        $idCuestionario = 0;
        $fechaCrea = date('Y/m/d H:i');
        $UsuarioCrea = $_SESSION['id'];
        $obj_cuestionario = new cls_andru_cuestionario();
        $obj_cuestionario->setCondicion("ID_PuntoBCR='" . $pIdPuntoBCR . "' AND ID_Fase='" . $pIdFase . "' LIMIT 1");
     ...
     * Método que se utiliza para cambiar el estado del registro según el campo llave 
     */
    public function andru_cuestionario_cambiar_estado() {
        if (isset($_SESSION['nombre'])) {
            $obj_andru_cuestionario = new cls_andru_cuestionario();
            if ($_GET['Estado'] == 1) {
                $obj_andru_cuestionario->setEstado("0");
            } else {
                $obj_andru_cuestionario->setEstado("1");
            }
            $obj_andru_cuestionario->setCondicion("ID_Cuestionario='" . $_GET['ID_Cuestionario'] . "'");
            $obj_andru_cuestionario->cambiar_estado_andru_cuestionario();
            unset($obj_andru_cuestionario);
            header("location:/ORIEL/index.php?ctl=andru_cuestionario_listar");
        } else {
            $tipo_de_alerta = "alert alert-warning";
